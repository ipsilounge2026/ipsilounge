...
| university_guide | /university-guide | 대학모집요강 관리 |
...
- 대학별 입시 자료(시행계획·수시/정시 모집요강·입시결과·학종 가이드북·선행평가) 외부 URL을 한 곳에 모은 카드 페이지.
- 자료 출처 정책: 공시 의무 자료는 **대학어디가**(adiga.go.kr), 대학 고유 자료는 **대학 입학처**. 입시결과는 대교협·자체발표 값이 달라 양쪽 모두 제공.
- **파일 호스팅 0건** — URL 만 우리 DB 에 매핑. 저작권·유지보수 부담 최소화.
- 사용자 카드(`/university-guide`): 좌측 대학명+입학처 / 우측 주요 4개(시행계획·수시·학종 가이드북·정시) / 하단 부가 3개(결과 2종·선행평가). URL 없는 버튼은 비활성 표시.
- 관리자 페이지: 학년도별 CRUD + 학년도 일괄 복사 (대학명만 또는 URL 까지).
- 사용자 웹 Navbar: "입시라운지" 드롭다운 (입시뉴스 + 대학모집요강).
- 모바일 앱 홈 그리드 5번: "입시 라운지" 묶음 → 허브 화면(`/ipsi-lounge`)에서 두 자식 화면으로 분기.
- 비로그인 접근 가능 (입시 뉴스와 동일).
- 1차 범위: 단순 카드 + 학년도/검색. 즐겨찾기·자동 URL 점검 Cron 은 2차 분리.
- **전년도 입시결과 내부 페이지** (`/university-guide/result/[unvCd]/[year]`): 수집 프로그램 Excel 을
  관리자(`/adiga-results`)에서 업로드 → `adiga_admission_results` 테이블 적재 → 수시/정시 탭·전형명 필터·추이 그래프 제공.
  - **자료 출처(source) 구분**: `대교협`(대학어디가 공시) / `자체발표`(대학 입학처 발표) — 같은 전형도 값이 달라 분리 보관.
    카드의 "전년도 입시결과(대교협)" 버튼 → 기본, "(자체발표)" 버튼 → `?source=자체발표`. 업로드·교체·삭제 모두 (학년도+출처) 단위.
  - Excel 형식 3종 자동 감지 (표준 37컬럼 / 3시트 정규화·원시 / 신 정리 형식) + 교체 방식 전체/부분(대학 단위) 선택.

### 13. 기획서 영역 기능 (상세는 각 기획서 참조)
...
### 대학모집요강 (/api/university-guide, /api/admin/university-guide)
| 메서드 | 경로 | 기능 |
|--------|------|------|
| GET | /api/university-guide/?year=&search= | 대학모집요강 목록. 학년도 미지정 시 최신, available_years 동봉. 비로그인 접근 가능 |
| GET | /api/admin/university-guide/?year=&search= | 관리자 전체 목록 (비활성 포함) |
| POST | /api/admin/university-guide/ | 신규 생성 |
| PUT | /api/admin/university-guide/{id} | 수정 |
| DELETE | /api/admin/university-guide/{id} | 삭제 |
| POST | /api/admin/university-guide/bulk-copy | 학년도 일괄 복사 (대학명만 또는 URL까지) |
| POST | /api/admin/university-guide/{id}/mark-checked | URL 점검 완료 표시 (last_checked) |
...
### university_guides
대학모집요강 안내. `(university, year)` UNIQUE. 자료별 외부 URL 8개 필드(대학어디가 5 + 대학 입학처 3). 파일 호스팅 0건, URL 매핑만. `last_checked` 로 수동 점검 시각 기록.
