...
CLEAN_SCORE_HEADERS = {
    ...
    "환산점수50%": "conv_score_50",
    "환산점수70%": "conv_score_70",
}
...
def _collect_pct_candidates(
    ...
    정규화 형식('백분위 50% 국어')과 원시 형식('... 50% ... 과목별 백분위 ... 국') 모두 지원.
    """
...
def _parse_unified_sheet(sheet, sheet_name: str, year: int | None, filename: str) -> list[dict]:
    ...
    clean_cols: dict[str, list[int]] = {}
    for idx, h in enumerate(headers_norm):
        field = CLEAN_SCORE_HEADERS.get(h)
        if field:
            clean_cols.setdefault(field, []).append(idx)

    # 원시 형식 fallback: 50/70 cut 후보 (정확 일치 컬럼이 전혀 없을 때만 사용)
    use_raw_cut = not clean_cols
    ...
    for row in rows_iter:
        ...
        if clean_cols:
            ...
            for field, idxs in clean_cols.items():
                values[field] = _first_value(row, idxs, _to_float)
