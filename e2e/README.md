...
## 현재 커버리지 (Sprint 1+2+3+4, 2026-04-19)

| 파일 | 테스트 | 통과 |
|------|--------|------|
| `tests/01-login.spec.ts` | 로그인 폼 성공 / 잘못된 비밀번호 | 2/2 |
| `tests/02-consultation-entry.spec.ts` | 상담 유형 6개 렌더 / 학습 상담 전환 / 비로그인 리다이렉트 | 3/3 |
| `tests/03-auth-forms.spec.ts` | 회원가입 페이지 렌더 / 약관 미동의 시 제출 disabled / 비번 재설정 요청 | 3/3 |
| `tests/04-analysis.spec.ts` | 업로드 UI 렌더 / 파일 미선택 에러 / 존재하지 않는 analysis_id / 비로그인 리다이렉트 | 4/4 |
| `tests/05-surveys.spec.ts` | 고등 설문 / 예비고1 설문 / 선배 설문 / 만족도 (빈·가짜 booking_id) | 5/5 |
| `tests/06-survey-completion.spec.ts` | 고등 설문 완주 (API 주입+submit) / 예비고1 설문 완주 | 2/2 |
| `tests/07-consultation-booking.spec.ts` | counselor_a 슬롯 seed 조회 / 반복 조회 일관성 | 2/2 |
| `tests/08-booking-full.spec.ts` | student_t3 심리상담 booking 생성 → /consultation/my 확인 | 1/1 |
| `tests/09-satisfaction-cycle.spec.ts` | booking → admin complete → 만족도 설문 자동 생성 → patch → submit | 1/1 |

**합계: 23/23 pass** (로컬 검증 완료, 실행 시간 ≈47초)
...
- **Sprint 5** (예정): admin-web E2E (관리자 검수·상담 기록 작성·상태 전이), 결제 흐름 (토스페이먼츠 mock)
