...
## 현재 커버리지 (Sprint 1~5, 2026-04-19)

| 파일 | 타겟 | 테스트 | 통과 |
|------|------|--------|------|
| `tests/01-login.spec.ts` | user-web | 로그인 폼 성공 / 잘못된 비밀번호 | 2/2 |
| `tests/02-consultation-entry.spec.ts` | user-web | 상담 유형 6개 / 학습 상담 전환 / 비로그인 리다이렉트 | 3/3 |
| `tests/03-auth-forms.spec.ts` | user-web | 회원가입 렌더 / 약관 disabled / 비번 재설정 | 3/3 |
| `tests/04-analysis.spec.ts` | user-web | 업로드 UI / 파일 미선택 에러 / 존재 X analysis_id / 비로그인 | 4/4 |
| `tests/05-surveys.spec.ts` | user-web | 고등·예비고1·선배 설문 + 만족도 2 | 5/5 |
| `tests/06-survey-completion.spec.ts` | API | 고등 설문 완주 / 예비고1 설문 완주 | 2/2 |
| `tests/07-consultation-booking.spec.ts` | API | 슬롯 seed 조회 / 반복 조회 일관성 | 2/2 |
| `tests/08-booking-full.spec.ts` | API | 심리상담 booking 생성 → /my 확인 | 1/1 |
| `tests/09-satisfaction-cycle.spec.ts` | API | booking → admin complete → 만족도 submit | 1/1 |
| `tests/10-admin-dashboard.spec.ts` | **admin-web** | 인증 대시보드 / 비인증 리다이렉트 / /consultation / /login 폼 | 4/4 |
| `tests/11-admin-notes-review.spec.ts` | API + admin-web | 상담 기록 작성 + 검수 / 학생 상담 이력 조회 / /consultation/notes UI | 3/3 |

**합계: 30/30 pass** (로컬 검증 완료, 실행 시간 ≈1분 40초)

## 스프린트 로드맵

- **Sprint 1** (완료): 로그인 + 상담 진입 (2026-04-19)
- **Sprint 2** (완료): 회원가입·비번재설정, 학생부 업로드·리포트 차단, 설문 3종+만족도 (2026-04-19)
- **Sprint 3** (완료): 설문 완주(API patch+submit) + 상담 슬롯 seed 검증 (2026-04-19)
- **Sprint 4** (완료): 실제 booking 생성 + 만족도 설문 전체 사이클 (2026-04-19)
- **Sprint 5** (완료): admin-web E2E (인증/대시보드/상담 관리/로그인 폼) (2026-04-19)
- **Sprint 6** (완료): admin-web 상담 기록 작성·검수 흐름 (2026-04-19)

### 보류 중 (구현 선행 필요)
- 🔒 결제 E2E — 토스페이먼츠 구현 완료 후
- 🔒 FCM E2E — Google Play 등록 + Firebase 활성화 완료 후
- 🔒 analyzer 내신/입결/전형적합도 시트 E2E — 대학 DB 확보 + 시트 구현 완료 후

> 원칙: **구현 완료 기능 → 바로 E2E 추가**. 구현 없는 기능의 E2E 작성 금지.
